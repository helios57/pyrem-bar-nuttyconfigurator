...
async function generateDynamicSlotCommands(tweakFileCache, packIntoSlots, getSlotSummary) {
    ...
    const applyMaxThisUnitOverride = (sectionObj) => {
        ...
        return {
            ...
            encodedChars: calculateEncodedSizeForBuilder(updatedCode)
        };
    };
        ...
        if (multiplier && multiplier !== "" && multiplier !== "1") {  // Don't include 1x (no change)
            ...
            if (typeof window.generateLuaTweakRaw === 'function') {
                ...
                hpSections.push({
                    ...
                    encodedChars: calculateEncodedSizeForBuilder(luaCode),
    ...
    const raptorWaveValue = getSelectedRaptorWaveMode();
    ...
    if (!isScavengersMode && raptorWaveValue !== 'none') {
        ...
        } else if (raptorWaveValue === 'doom_mode') {
...
async function generateCommandsImpl() {
    ...
    const isDoomMode = !isScavengers && getSelectedRaptorWaveMode() === 'doom_mode';

    let renameCommand = isScavengers
        ? `$rename PvE [PMod] NuttyB Scavengers `
        : isDoomMode
            ? `$rename PvE [PMod] NuttyB Doom `
            : `$rename PvE [PMod] NuttyB Raptors `;
    const renameParts = [];
